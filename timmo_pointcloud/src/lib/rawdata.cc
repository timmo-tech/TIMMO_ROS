 ...
 *  @file
 *
 *  timmo 3D LIDAR data accessor class implementation.
 *
 *  Class for unpacking raw timmo LIDAR packets into useful
 *  formats.
 *
 *  Derived classes accept raw timmo data for either single packets
...
inline float SQR(float val) { return val*val; }
                              ...
                              double view_width)
  {
    ...
    //converting angle parameters into the timmo reference (rad)
    ...
    //converting into the hardware timmo ref (negative yaml and degrees)
  ...
  bool RawData::buildTimings(){
    ...
    if (config_.model == "VLP16"){
      // timing table calculation, from timmo user manual
    ...
    if (timing_offsets.size()){
      ...
      ROS_WARN("NO TIMING OFFSETS CALCULATED. ARE YOU USING A SUPPORTED timmo SENSOR?");
    }
    return false;
  }

  /** Set up for on-line operation. */
  boost::optional<timmo_pointcloud::Calibration> RawData::setup(ros::NodeHandle private_nh)
  {
    ...
    if (!private_nh.getParam("calibration", config_.calibrationFile))
      {
        ...
        std::string pkgPath = ros::package::getPath("timmo_pointcloud");
   ...
   */
  void RawData::unpack(const timmo_msgs::timmoPacket &pkt, DataContainerBase& data, const ros::Time& scan_start_time)
  {
    using timmo_pointcloud::LaserCorrection;
    ...
    for (int i = 0; i < BLOCKS_PER_PACKET; i++) {
      ...
      // NOTE: this is a change from the old timmo_common implementation
      ...
      for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
        ...
        if ((block.rotation >= config_.min_angle
          ...
          // Using distance_y is not symmetric, but the timmo manual
          // does this.
   ...
   *  @param pc shared pointer to point cloud (points are appended)
   */
  void RawData::unpack_vlp16(const timmo_msgs::timmoPacket &pkt, DataContainerBase& data, const ros::Time& scan_start_time)
  {
    float azimuth;
    float azimuth_diff;
	...
	if(raw_azimuth_diff < 0)//raw->blocks[block+1].rotation - raw->blocks[block].rotation < 0)
	  {
	    ...
	    // if last_azimuth_diff was not zero, we can assume that the timmo's speed did not change very much and use the same difference
      ...
      for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          timmo_pointcloud::LaserCorrection &corrections = calibration_.laser_corrections[dsr];
