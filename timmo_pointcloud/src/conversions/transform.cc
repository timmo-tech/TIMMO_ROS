...
namespace timmo_pointcloud
{
  ...
  Transform::Transform(ros::NodeHandle node, ros::NodeHandle private_nh, std::string const & node_name):
    ...
    data_(new timmo_rawdata::RawData),
    ...
    diagnostics_(node, private_nh, node_name)
  {
    ...
    config_.target_frame = config_.fixed_frame = "timmo";
    ...
    // subscribe to timmoScan packets using transform filter
    timmo_scan_.subscribe(node, "timmo_packets", 10);
    tf_filter_ptr_ = boost::shared_ptr<tf::MessageFilter<timmo_msgs::timmoScan> >(
            new tf::MessageFilter<timmo_msgs::timmoScan>(timmo_scan_, *tf_ptr_, config_.target_frame, 10));
    ...
    // Diagnostics
    diagnostics_.setHardwareID("timmo Transform");
  ...
  void Transform::reconfigure_callback(
      timmo_pointcloud::TransformNodeConfig &config, uint32_t level)
  {
   ...
   */
  void
    Transform::processScan(const timmo_msgs::timmoScan::ConstPtr &scanMsg)
  {
