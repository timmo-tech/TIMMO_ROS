...
/** @file
    ...
    This class converts raw timmo 3D LIDAR packets to PointCloud2.
...
#include <timmo_pointcloud/organized_cloudXYZIR.h>

namespace timmo_pointcloud
{
  /** @brief Constructor. */
  Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh, std::string const & node_name):
    data_(new timmo_rawdata::RawData()), first_rcfg_call(true),
    diagnostics_(node, private_nh, node_name)
  {
    // Get startup parameters
    private_nh.param<std::string>("fixed_frame", config_.fixed_frame, "timmo");
    private_nh.param<std::string>("target_Frame", config_.target_frame, "timmo");
    ...
    boost::optional<timmo_pointcloud::Calibration> calibration = data_->setup(private_nh);
    if(calibration)
    {
    ...
    output_ =
      node.advertise<sensor_msgs::PointCloud2>("timmo_points", 10);

    srv_ = boost::make_shared <dynamic_reconfigure::Server<timmo_pointcloud::
      CloudNodeConfig> > (private_nh);
    dynamic_reconfigure::Server<timmo_pointcloud::CloudNodeConfig>::
      CallbackType f;
    f = boost::bind (&Convert::callback, this, _1, _2);
    srv_->setCallback (f);

    // subscribe to timmoScan packets
    timmo_scan_ =
      node.subscribe("timmo_packets", 10,
                     &Convert::processScan, (Convert *) this,
                     ros::TransportHints().tcpNoDelay(true));

    // Diagnostics
    diagnostics_.setHardwareID("timmo Convert");
    // Arbitrary frequencies since we don't know which RPM is used, and are only
    // concerned about monitoring the frequency.
    diag_min_freq_ = 2.0;
    diag_max_freq_ = 20.0;
    using namespace diagnostic_updater;
    diag_topic_.reset(new TopicDiagnostic("timmo_points", diagnostics_,
  ...
  void Convert::callback(timmo_pointcloud::CloudNodeConfig &config,
                uint32_t level)
  {
  ...
  void Convert::processScan(const timmo_msgs::timmoScan::ConstPtr &scanMsg)
  {
